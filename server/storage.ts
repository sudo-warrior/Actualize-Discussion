...
export interface IStorage {
  ...
  revokeApiKey(userId: string, keyId: string): Promise<ApiKey | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async createApiKey(data: InsertApiKey): Promise<ApiKey> {
    const [created] = await db.insert(apiKeys).values(data as any).returning();
    return created;
  }
  ...
  async findApiKeyByHash(keyHash: string): Promise<ApiKey | undefined> {
    const [key] = await db.select().from(apiKeys).where(
      and(eq(apiKeys.keyHash, keyHash), eq(apiKeys.revoked, false))
    );
    return key;
  }

  async revokeApiKey(userId: string, keyId: string): Promise<ApiKey | undefined> {
    const [updated] = await db
      .update(apiKeys)
      .set({ revoked: true })
